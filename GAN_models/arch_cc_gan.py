...
import tensorflow as tf
from keras import backend as k
from custom_keras_layers import linear_kernel, clip_layer, mean_zero_layer, UpSampling_2Dcubical
...
def eeg_generator():
    ...
    model.add(mean_zero_layer())

    noise = Input(shape=(120,))
    input_label = Input(shape=(1,), dtype='int32')
    label_embedding = Flatten()(Embedding(n_classes, 120)(input_label))
    ...
    return Model([noise, input_label], eeg)
...
def eeg_discriminator():
    ...
    validity = (Dense(1, kernel_initializer='he_normal'))(features)
    label = (Dense(2, kernel_initializer='he_normal', activation="softmax"))(features)

    model.summary()

    return Model(eeg, [validity, label])
